...
int bestRank(std::string hand) {
    const std::string cards = "23456789TQKA";

    auto pos = hand.find("J");
    if (pos == std::string::npos)
        return handRank(hand);
    int maxrank = 0;
    for (auto const &c : cards) {
        hand[pos] = c;
        maxrank = std::max(maxrank, bestRank(hand));
    }
    return maxrank;
}
...
struct cmpByStringRank {
    bool operator()(const std::string& a, const std::string& b) const {
        auto result = bestRank(a) <=> bestRank(b);
        if (result < 0)
            return true;
        if (result == 0)
